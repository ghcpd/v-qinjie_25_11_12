...
def load_json_file(path):
    """Load a JSON file trying several encodings to be robust on Windows."""
    ...
    for enc in encodings:
        try:
            ...
            pass
    # If all fail, raise descriptive exception
...
def run_test_case(test_case):
    ...
    try:
        ...
        # detection heuristics: vulnerability success conditions
        # Additional evidence from raw server logs (look for RAW_SQL or RAW_AUTH_SQL in Project_A logs)
        try:
            logs_text = open(ROOT / 'logs' / 'server.log','r',encoding='utf-8').read()
            if 'RAW_SQL' in logs_text:
                result['evidence'].append('RAW_SQL_logged')
            if 'RAW_AUTH_SQL' in logs_text:
                result['evidence'].append('RAW_AUTH_SQL_logged')
        except Exception:
            pass
